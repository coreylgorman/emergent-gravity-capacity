...
\begin{abstract}
\textbf{Idea in one line:} Gravity responds not only to mass--energy but also to the \emph{finite capacity} of spacetime to store quantum information. As the universe expands, the \emph{capacity occupation} increases monotonically (an entropic statement), which throttles the effective strength of gravity in a predictable, testable way.

\smallskip
\textbf{Three pillars.}
(1) \emph{Capacity occupation governs gravity}: as spacetime's finite capacity for quantum information becomes more \emph{occupied}, the effective strength of gravity is reduced. Conversely, when more capacity remains \emph{available}, gravity operates closer to its full GR strength.\\
...
Think of a finite-bandwidth channel. When it is \emph{lightly occupied}, the channel behaves as if it had more headroom; under \emph{heavy occupation} it throttles. Our claim is that spacetime has an \emph{information capacity} that plays a similar role. The cosmic state variable $\eps(a)$ (dimensionless; ``capacity occupation'') \emph{monotonically increases} with the scale factor $a$:
\[
\frac{d\eps}{d\ln a} \ge 0 \qquad \text{(monotonic entropy / capacity occupation increase).}
\]
This occupation renormalizes the effective Planck mass and therefore the effective gravitational coupling.
...
\begin{quote}
\textbf{Callout:} More \emph{occupied} capacity $\Rightarrow$ weaker effective gravity. More \emph{available} capacity $\Rightarrow$ gravity closer to GR strength.
\end{quote}
...
\begin{itemize}
\item \textbf{Capacity occupation governs $G$.} More occupied capacity weakens effective gravity; more available capacity leaves gravity close to GR strength.
...
\begin{enumerate}
\item $\eps(a)$ increases monotonically (entropy/capacity occupation $\nearrow$ with cosmic time).
