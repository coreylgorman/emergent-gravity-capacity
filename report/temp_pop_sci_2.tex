...
\medskip
\paragraph*{(E) Natural $H_0$ around 70 (resolving the tension).}
The framework naturally favors $H_0 \approx 70$ km/s/Mpc as the ``anchor'' value emerging from the capacity budget. This aligns with time-resolved standard-bubble (TRSB) measurements near 70. In this view:
\begin{itemize}
\item The \textbf{SHOES ladder} pulls the value higher ($\sim 73$) due to calibration assumptions.
\item The \textbf{Planck CMB inference} pulls the value lower ($\sim 67$) because it extrapolates $\Lambda$CDM without capacity throttling.
\end{itemize}
The true late-time cosmology sits naturally between them, with capacity throttling enforcing an anchor near 70.
