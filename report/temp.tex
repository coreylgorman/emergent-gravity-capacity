...
\noindent\textbf{Reproducibility.} For the MI/moment‑kill weights used in our numerics (App.~\ref{app:MI}), the projected integral evaluates to
\[
I_{00}=3.932017\ \ \text{(dimensionless)},
\]
so with \(C_T=1/(120\pi^2)\) one obtains \(\beta=2\pi C_T I_{00}=0.02086\) as quoted. The helper script \texttt{beta\_methods\_v2.py} prints this \(I_{00}\) value.
...
\end{proposition}
\noindent\emph{Operational diagnostic.} The routine \texttt{referee\_pipeline.py} reports a scalar residual \(R_{\rm nonloc}\equiv\sum_{i\neq 0}\big|\bar\chi^{\rm proj}(\Delta_i)\big|\,\Delta(\ln a)_i\) outside the contact bin; a statistically significant non‑zero \(R_{\rm nonloc}\) falsifies (H1).
...
\]
Imposing \(F_g(\chi_\odot)\le \epsilon_{\rm SS}=10^{-5}\) with \(q=2\) implies
\[
\chi_\star \lesssim \chi_\odot\,\epsilon_{\rm SS}^{1/2} \approx 9.2\times 10^{2}.
\]
A representative choice \(\chi_\star=900\), \(q=2\) then yields \(F_g(\chi_\odot)\approx 9.6\times 10^{-6}\), while leaving cosmological environments (\(\chi_g\ll\chi_\star\)) essentially unsuppressed (\(F_g\simeq 1\)). For transparency we report a small compliance table:
\begin{center}
\begin{tabular}{lcccc}
\toprule
$\chi_\star$ & 1200 & 1000 & 900 & 800\\
\midrule
$F_g(\chi_\odot;\,q{=}2)$ & $1.7\times 10^{-5}$ & $1.18\times 10^{-5}$ & $9.6\times 10^{-6}$ & $7.6\times 10^{-6}$\\
\bottomrule
\end{tabular}
\end{center}
This envelope is a Solar–System compliance switch rather than a cosmology‑level fit; other monotone forms with \(q\in[1,3]\) remain viable and will be data‑constrained.
...
\paragraph{Segment ratio \(5/16\).}
With the normalized weight \(\hat\rho(u)=\tfrac{3}{4}(1-u^2)\) on \(u\in[-1,1]\), the CHM segment average reads
\[
R_{\rm seg}=\int_{-1}^{1}\! \hat\rho(u)\,\mathcal I(u)\,du=\frac{5}{16},
\]
where \(\mathcal I(u)\) is the (unit‑normalized) generator density along the wedge family. Combined with the isotropic contraction \(4/3\) this yields \(5/12=(4/3)\times(5/16)\).
