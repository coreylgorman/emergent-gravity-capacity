...
\subsection{Assumption D\texorpdfstring{$'$}{'} (optional; shock-selective optical channel)}
\label{sec:lemmaDprime}

\noindent\textbf{Motivation.} In cluster mergers with strong shocks, observed weak-lensing peaks often coincide with the collisionless galaxies rather than with the shocked gas. The baseline closure (\(\Sigma\simeq 1\)) preserves distances and attributes modifications to growth; to account for Bullet-like systems within first principles, we introduce an \emph{optional} local, algebraic anisotropic-stress channel that reduces the \emph{lensing} combination \((\Phi+\Psi)\) \emph{only} in high-shear shocked gas, while leaving FRW and laminar flows unaffected.

\smallskip
\noindent\textbf{Ingredients.} Let \(u^\mu\) be the baryon four-velocity and define the projector \(h_{\mu\nu}=g_{\mu\nu}+u_\mu u_\nu\), expansion \(\theta=\nabla_\alpha u^\alpha\), and shear
\[
\sigma_{\mu\nu}\equiv h_\mu^{\ \alpha} h_\nu^{\ \beta}\left(\nabla_{(\alpha} u_{\beta)}-\tfrac{1}{3}\theta\,h_{\alpha\beta}\right),\qquad
\mathcal S_{\rm shock}\equiv \ell^2\,\sigma_{\mu\nu}\sigma^{\mu\nu}\ge 0.
\]
Introduce a heavy, traceless auxiliary tensor \(Q_{\mu\nu}\) with algebraic potential
\be
...
\smallskip
\noindent\textbf{Quasi-static limit (cluster scales).} Linearized Einstein equations (sub-horizon) acquire
\bse
...
\smallskip
\noindent\textbf{Local, saturating form for \(\Sigma\).} In practice we summarize Eq.~\eqref{eq:qs-lensing-Dp} by a local interpolation
\be
\label{eq:sigmalocal}
\Sigma(x)\;\simeq\;1\;-\;\alpha_{\rm opt}\,\frac{\mathcal S_{\rm shock}(x)}{1+\mathcal S_{\rm shock}(x)},\qquad 0<\alpha_{\rm opt}<1,
\ee
which yields \(\Sigma\to 1-\alpha_{\rm opt}\) in very strong shocks and \(\Sigma\to 1\) away from shocks. The growth coupling \(\mu(\varepsilon,s)\) is unchanged (still set by \(M_*^2\)).
...
\paragraph{Safety checks.} \(c_T=1\) (no curvature-derivative couplings), FRW \(\Sigma\simeq1\) (\(\sigma_{\mu\nu}=0\)), Solar System unaffected, and the integrated GW/EM split remains \(\ll 10^{-3}\) since the effect is cluster-local. Positivity is manifest from the squared potential in \(\mathcal L_Q\).
\vspace{1ex}
...
\paragraph{Merging clusters (optional D$'$).}
...
\label{sec:program}
\textbf{Lemma A} (diamond KMS control): scaling proven, sharp bounds left to microlocal analysis. \textbf{Lemma B} (projector universality): established. \textbf{Assumption C} and \textbf{Assumption D}: stated here with rationale; proofs deferred (Secs.~\ref{sec:lemmaC}, \ref{sec:lemmaD}). \textbf{Assumption D$'$} (shock-selective optical channel): optional extension for merging clusters (Sec.~\ref{sec:lemmaDprime}). \textbf{Lemma E} (FDT positivity): follows from BKM positivity. \textbf{Lemma F} (geometric \(5/12\)): derived.\\
...
\item \textbf{Shock-selective optics (Assumption D$'$):} calibrate $\alpha_{\rm opt}$ and $\kappa_{\rm opt}$ from simulations; test morphology predictions (A–D); bound degeneracies with baryonic microphysics; derive microscopic origin of the shear coupling.
...
We submit the revised manuscript “Modular Response in Free Quantum Fields: A KMS/FDT Theorem and Conditional Extensions” (dated August 30, 2025). This revision incorporates the lensing-focused minor revisions recommended by the referee and strengthens the Bullet Cluster analysis:

\begin{itemize}
  \item In Sec.~\ref{sec:lemmaDprime} we quantify the calibration window $\alpha_{\rm opt}\sim 0.6$–$0.8$ via a back-of-the-envelope centroid-shift argument tied to Bullet-like offsets and strong-shock limits ($\Sigma\simeq 1-\alpha_{\rm opt}$).
  \item In Sec.~\ref{sec:obs} (Merging clusters) we explicitly cite Bullet Cluster observations \cite{Clowe2006,Markevitch2002} and radio relic/shock correlations \cite{vanWeeren2019}; we mention Abell~520 as a comparative testbed \cite{Mahdavi2007}.
  \item In Sec.~\ref{sec:data} we document compatible shock-finders for \texttt{cluster\_optics\_hook.py} (velocity-jump, pressure/temperature-jump, Godunov-flux).
  \item In Sec.~\ref{sec:program} we add a falsifier specific to D$'$: persistent offsets in low-Mach or unshocked systems inconsistent with the $\mathcal S_{\rm shock}$ scaling.
\end{itemize}

\noindent \textbf{Falsifiability:} We emphasize explicit failure modes (Sec.~\ref{sec:program}): GW/EM distance splits, bounds on $|\dot G/G|$, an $\ell^4\log\ell$ signal in the MI/moment-kill channel, violations of the FRW budget normalization, and (for D$'$) lack of correlation with shock diagnostics or offsets persisting in low-$\mathcal M$ systems.

\noindent \textbf{Data \& Code:} All analysis scripts and single-file runners are archived on Zenodo (Zenodo DOI: \texttt{\zenododoi}). The archive includes \texttt{beta\_methods\_v2.py}, \texttt{cosmology\_runner.py}, \texttt{referee\_pipeline.py}, \texttt{fv\_semi\_analytic.py}, \texttt{gadget4\_mu\_eps\_toy.py}, \texttt{s8\_hysteresis\_run.py}, and \texttt{cluster\_optics\_hook.py}, each with usage notes to reproduce the figures and tables.
