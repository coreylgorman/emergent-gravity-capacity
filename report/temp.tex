...
We submit the revised manuscript “Modular Response in Free Quantum Fields: A KMS/FDT Theorem and Conditional Extensions” (dated August 30, 2025). This revision implements the minor updates recommended by the referee and aligns with Referee 7’s request for stronger physical foundations and explicit falsifiers:

\begin{itemize}
  \item We promote the \emph{Frame-independence of throttling} to a subsection (Sec.~\ref{sec:throttle-remark}) with a covariant, action-level realization of the environment modulation $begin:math:text$s(x)$end:math:text$ and an explicit Jordan$begin:math:text$\\leftrightarrow$end:math:text$Einstein map.
