...
% —— New hyperlinked figure: Computation & Validation Pipeline ——
\begin{figure*}[t]
\centering
\begin{tikzpicture}[
  node distance=7mm and 12mm,
  every node/.style={font=\small},
  box/.style={draw, rounded corners, align=center, inner sep=4pt, outer sep=2pt, fill=white},
  arr/.style={-Latex, thick}
]
% Left column: beta computation
\node[box, fill=blue!5] (chm) {\textbf{CHM/OP modular data}\\ \hyperref[sec:beta-calc]{Sec.~\ref*{sec:beta-calc}}};
\node[box, below=of chm] (mi) {MI subtraction +\\ moment-kill\\ \hyperref[app:MI-momentkill]{App.~\ref*{app:MI-momentkill}}};
\node[box, below=of mi] (routes) {4 independent routes\\ (real, spectral, Eucl., replica)\\ \hyperref[sec:beta-multimethod]{Sec.~\ref*{sec:beta-multimethod}}};
\node[box, below=of routes, fill=green!7] (beta) {$\beta$ plateau \& stability\\ \hyperref[sec:convergence]{Sec.~\ref*{sec:convergence}}};

% Middle: geometric normalization
\node[box, right=2.6cm of mi, fill=yellow!10] (f) {Geometric normalization $f$\\ (schemes A/B; angle-invariant)\\ \hyperref[sec:f-norm]{Sec.~\ref*{sec:f-norm}}};
\node[box, below=of f, fill=yellow!10] (cgeo) {$\cgeo(\theta)$, unit-solid-angle\\ \hyperref[app:cgeo-integral]{App.~\ref*{app:cgeo-integral}}};
\node[box, below=of cgeo, fill=orange!15] (omega) {\Large $\OmL=\beta f \cgeo$\\ \hyperref[sec:OmegaL]{Sec.~\ref*{sec:OmegaL}}};

% Right column: validations and applications
\node[box, right=2.6cm of routes, fill=blue!5] (substr) {Substrate checks:\\ HQTFIM \& Gaussian\\ \hyperref[sec:substrate-validations]{Sec.~\ref*{sec:substrate-validations}}};
\node[box, below=of substr] (state) {Entropic state-action $\eps(a)$\\ \hyperref[sec:state-action]{Sec.~\ref*{sec:state-action}}};
\node[box, below=of state] (growth) {Growth $f\sigma_8$, $S_8$\\ \hyperref[sec:growth]{Sec.~\ref*{sec:growth}}};
\node[box, below=of growth] (h0) {Hubble ladder (capped/uncapped)\\ \hyperref[sec:h0-illustration]{Sec.~\ref*{sec:h0-illustration}}};

% Arrows
\draw[arr] (chm) -- (mi);
\draw[arr] (mi) -- (routes);
\draw[arr] (routes) -- (beta);
\draw[arr] (mi.east) -- (f.west);
\draw[arr] (f) -- (cgeo);
\draw[arr] (cgeo) -- (omega);
\draw[arr] (routes.east) -- (substr.west);
\draw[arr] (omega.east) to[bend left=12] (state.west);
\draw[arr] (substr) -- (state);
\draw[arr] (state) -- (growth);
\draw[arr] (growth) -- (h0);
\end{tikzpicture}
...
% —— New hyperlinked figure: CGM → compensator → mapping ——
\begin{figure*}[t]
\centering
\begin{tikzpicture}[
  node distance=7mm and 12mm,
  every node/.style={font=\small},
  box/.style={draw, rounded corners, align=center, inner sep=4pt, outer sep=2pt, fill=white},
  obs/.style={draw, rounded corners, align=center, inner sep=4pt, outer sep=2pt, fill=red!5},
  good/.style={draw, rounded corners, align=center, inner sep=4pt, outer sep=2pt, fill=green!7},
  arr/.style={-Latex, thick}
]
\node[obs] (cgm) {\textbf{CGM obstruction} ($\Delta\!\le\! d/2$)\\ contact \& log terms\\ \hyperref[sec:CGM]{Sec.~\ref*{sec:CGM}}};
\node[box, right=2.8cm of cgm] (mi) {MI subtraction + moment-kill\\ removes area/contact\\ \hyperref[app:MI-momentkill]{App.~\ref*{app:MI-momentkill}}};
\node[box, below=of mi] (safe) {\emph{Safe window}\\ $\epsilon_{\rm UV}\!\ll\!\ell\!\ll\!\min\{L_{\rm curv},\lambda_{\rm mfp},m_i^{-1}\}$\\ \hyperref[def:safe-window]{Def.~\ref*{def:safe-window}}};
\node[box, right=2.8cm of mi, fill=yellow!10] (run) {Slow running $M^2(\eps)$\\ adds focusing source\\ \hyperref[sec:clausius-vs-jacobson]{Sec.~\ref*{sec:clausius-vs-jacobson}}};
\node[good, below=of run] (cancel) {Log compensator cancels\\ marginal $\ell^4\log(\ell)$ term\\ \hyperref[prop:marginal]{Prop.~\ref*{prop:marginal}}};
\node[good, right=2.8cm of run] (epmr) {\textbf{EPMR} in small diamonds:\\ finite $\ell^4$ = flat-space value\\ \hyperref[app:epmr]{App.~\ref*{app:epmr}}, Thm.~\ref{thm:A2-working}};
\node[box, below=of epmr, fill=orange!15] (map) {\Large $\OmL=\beta f \cgeo$\\ \hyperref[sec:OmegaL]{Sec.~\ref*{sec:OmegaL}}};

\draw[arr] (cgm) -- (mi);
\draw[arr] (mi) -- (run);
\draw[arr] (mi) -- (safe);
\draw[arr] (run) -- (cancel);
\draw[arr] (cancel) -- (epmr);
\draw[arr] (epmr) -- (map);
\end{tikzpicture}
