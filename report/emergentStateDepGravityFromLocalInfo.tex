...
\end{document}\documentclass[aps,prd,onecolumn,superscriptaddress,nofootinbib]{revtex4-2}
...
\vspace{2mm}
% —— Conceptual flow figure (A2 domain, CGM, and resolution) ——
\begin{figure*}[t]
\centering
\begin{adjustbox}{max width=\textwidth, max height=0.9\textheight}
\begin{tikzpicture}[
  ...
  every node/.style={font=\footnotesize},
  head/.style={draw, rounded corners=2pt, align=center, inner sep=4pt, outer sep=0pt, text width=0.78\linewidth, font=\footnotesize\bfseries},
  box/.style={draw, rounded corners=2pt, align=center, inner sep=4pt, outer sep=0pt, text width=0.78\linewidth},
  arr/.style={-Latex, semithick, shorten >=2pt, shorten <=2pt}
]
% Blue — QFT layer
\node[head, draw=flowBlue!80, fill=flowBlue!20] (QH) {[Blue: QFT Layer]};
\node[box, below=of QH, draw=flowBlue!80, fill=flowBlue!8] (Q1) {Vacuum state in QFT};
\node[box, below=of Q1, draw=flowBlue!80, fill=flowBlue!8] (Q2) {Map CHM ball $\to$ small causal diamond (local Rindler wedge approximation)};
\node[box, below=of Q2, draw=flowBlue!80, fill=flowBlue!8] (Q3) {$\rho_A = \mathrm{Tr}_{\text{outside}}\,|0\rangle\!\langle 0|$};
\draw[arr] (QH) -- (Q1); \draw[arr] (Q1) -- (Q2); \draw[arr] (Q2) -- (Q3);

% Red — Thermal layer
\node[head, below=6mm of Q3, draw=flowRed!80, fill=flowRed!20] (TH) {[Red: Thermal Layer]};
\node[box, below=of TH, draw=flowRed!80, fill=flowRed!8] (T1) {Thermal form: $\rho_A = e^{-K}/Z$ (dimensionless modular $K$)};
\node[box, below=of T1, draw=flowRed!80, fill=flowRed!8] (T2) {Unruh temperature: $T_U = \hbar a/(2\pi c k_B)$};
\draw[arr] (TH) -- (T1); \draw[arr] (T1) -- (T2);

% Green — Entropy layer
\node[head, below=6mm of T2, draw=flowGreen!80, fill=flowGreen!20] (EH) {[Green: Entropy Layer]};
\node[box, below=of EH, draw=flowGreen!80, fill=flowGreen!8] (E1) {von Neumann entropy: $S_A = -\mathrm{Tr}(\rho_A \log \rho_A)$};
\node[box, below=of E1, draw=flowGreen!80, fill=flowGreen!8] (E2) {Entanglement first law: $\delta S = \delta\langle K\rangle = (1/T_U)\,\delta\langle H\rangle$ (boost flow)};
\node[box, below=of E2, draw=flowGreen!80, fill=flowGreen!8] (E3) {Clausius relation in safe window: $\delta Q = T_U\,\delta S + \mathcal O(\ell^6)$ \,(A2; \hyperref[thm:A2-working]{Thm.~\ref*{thm:A2-working}})};
\draw[arr] (EH) -- (E1); \draw[arr] (E1) -- (E2); \draw[arr] (E2) -- (E3);

% Orange — CGM obstruction
\node[head, below=6mm of E3, draw=flowOrange!90, fill=flowOrange!25] (OH) {[Orange: CGM 2016 Obstruction]};
\node[box, below=of OH, draw=flowOrange!90, fill=flowOrange!12] (O1) {For $\Delta \le d/2$: obstruction $\delta\!\langle K\rangle \sim \ell^d\log\ell$ (breaks Clausius; \hyperref[sec:CGM]{Sec.~\ref*{sec:CGM}})};
\draw[arr] (OH) -- (O1);

% Purple — Resolution
\node[head, below=6mm of O1, draw=flowPurple!80, fill=flowPurple!20] (RH) {[Purple: Resolution in this work]};
\node[box, below=of RH, draw=flowPurple!80, fill=flowPurple!8] (R1) {MI subtraction + moment-kill $\Rightarrow$ cancel $r^0, r^2$ contacts; safe window $\Rightarrow$ curvature $\mathcal O(\ell^6)$ (\hyperref[app:epmr]{App.~\ref*{app:epmr}})};
\node[box, below=of R1, draw=flowPurple!80, fill=flowPurple!8] (R2) {Marginal $\Delta{=}d/2$: slow running $M^2(\varepsilon)$ adds focusing source $k^a k^b\nabla_a\nabla_b M^2$ canceling the log (\hyperref[sec:clausius-vs-jacobson]{Sec.~\ref*{sec:clausius-vs-jacobson}})};
\draw[arr] (RH) -- (R1); \draw[arr] (R1) -- (R2);

% Restored identity (highlight)
\node[box, below=6mm of R2, draw=flowGreen!80, very thick, fill=flowGreen!15] (C1) {\textbf{Restored working-order Clausius identity:} $\delta\!\langle K_{\rm sub}\rangle=(2\pi C_T I_{00})\,\ell^4\,\delta\varepsilon + \mathcal O(\ell^6)$ \\ (flat-space finite coefficient; \hyperref[thm:A2-working]{Theorem~\ref*{thm:A2-working}})};

\end{tikzpicture}
\end{adjustbox}
\caption{Conceptual flow from QFT vacuum reduction to the working-order Clausius identity, highlighting the CGM (2016) obstruction and the resolution used here. Colors match later sections; placed early to orient readers.}
