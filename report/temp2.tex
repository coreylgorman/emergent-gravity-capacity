...
\begin{corollary}[Conditional background statement]
...
\noindent\textbf{FRW budget (conditional).}
Under the same averaging and reparameterization, the spatially averaged, projected retarded kernel admits
\[
\Big\langle \!\int \chi^{\rm proj}_{QK}(a,a')\,d^3x \Big\rangle
= \beta\, f\, \cgeo\, \delta(\ln a-\ln a') + \ldots
\]
so integrating the entropy-driven evolution $d\eps/d\ln a=\sigma(a)I(a)\ge0$ yields the coarse-grained identity
\be
\int_{a_i}^{1}\!\eps(a)\,d\ln a \;=\; \OmL \;=\; \beta\, f\,\cgeo,
\label{eq:budget}
\ee
which we use as a normalization \emph{conditional} on locality of the averaged kernel, adiabatic evolution through matter domination, and small CHM vs. half-space KMS deviation (App.~\ref{app:chm-kms-estimate}).
\smallskip
...
Assuming the conditional background relation \(\OmL=\beta f\cgeo=0.685\pm 0.034\), the previously quoted illustrative shifts \(H_0: 73.0\to 71.18\) (uncapped SN) and \(\to 70.89\) (capped SN+Cepheid) acquire \(\pm 0.17\)~km/s/Mpc systematic envelopes from \(\beta\), reported as
\be
