...
\smallskip
\emph{What is new.} (i) Completed proofs in the Gaussian/Hadamard sector, including a covariant KMS\(\to\)FRW averaging lemma with explicit error budget; (ii) \textbf{Assumptions C and D stated with rationale} (relative entropy \(\leftrightarrow\) canonical energy in the projected diamond; uniqueness of \(M^2\) at working order), with proofs deferred; (iii) semi-analytic quantification of the safe-window volume fraction \(f_V(\ell_{\min})\) with Press--Schechter/Sheth--Tormen inputs; (iv) a symmetry-constrained \(F_g\) envelope with calibrated \(\chi_\star\) and \(q\); (v) uncertainty propagation of \(\beta\) into \(S_8\) and \(H_0\) bounds; (vi) a preliminary entropic derivation (App.~\ref{app:entropic-proof}) linking KMS positivity to FRW evolution, pending validation. Part II remains explicitly labeled as conditional. Part II relies on unproven extensions to interacting QFTs, which may fail pending further theoretical development.
\end{abstract}
...
\label{eq:mapping}
\ee
\noindent \emph{Weak-field acceleration (toy/conditional).} Using the universal \(5/12\) prefactor and the background normalization \(\OmL=\beta f\cgeo\), the weak-field normalization implies a MOND-like acceleration scale
\be
...
\subsection{Assumption C (stated): Relative entropy \texorpdfstring{$\leftrightarrow$}{<->} canonical energy in the projected diamond}
\label{sec:lemmaC}
...
\subsection{Assumption D (stated): Uniqueness of the \texorpdfstring{$M^2$}{M^2} coupling at working order}
\label{sec:lemmaD}
...
\ee
A preliminary entropic derivation relating this monotonicity to Araki relative entropy is given in App.~\ref{app:entropic-proof}.
...
\textbf{Lemma G (Nonlinear validation)}: Full Gadget-4 simulations are in progress to test \(\mu(\varepsilon)\) and \(F_g(\chi_g)\) effects on structure formation and lensing.\\
...
Consider a causal diamond (CHM) with radius \(\ell\) in a locally Hadamard state \(\rho\), and reference state \(\sigma\) (vacuum-equivalent). The MI/moment-kill projector yields \(\delta\langle K_{\rm sub}\rangle=\beta\,\ell^4\,\delta\varepsilon+\mathcal O(\ell^6)\) (Sec.~\ref{sec:theorem}).

\paragraph{Step 2: Araki relative entropy.}
For a smooth path \(\rho(\lambda)\) with \(\rho(0)=\sigma\), the second variation satisfies (Assumption C, Sec.~\ref{sec:lemmaC})
\[
\left.\frac{d^2}{d\lambda^2}\right|_0 S(\rho(\lambda)\Vert\sigma)
=\iint \chi^{\rm proj}_{QK}(x,x')\,\delta Q(x)\,\delta K_{\rm sub}(x')\,d^4x\,d^4x' \ \ge 0,
\]
with \(\chi^{\rm proj}_{QK}\ge 0\) by KMS/FDT positivity (Sec.~\ref{sec:theorem}).

\paragraph{Step 3: Modular response.}
With \(\delta K_{\rm sub}\propto \delta\varepsilon\) (Def.~\eqref{eq:eps-def}), positivity reflects entropic increase in the projected channel.

\paragraph{Step 4: FRW mapping.}
The KMS\(\to\)FRW reparametrization (\(s\to\ln a\), Sec.~\ref{sec:kms-frw}) gives, schematically,
\[
\frac{dS}{d\ln a}\ \propto\ \frac{dS}{ds}\,\frac{ds}{d\ln a}\ \ge 0,
\]
under the adiabatic and analyticity assumptions of Sec.~\ref{sec:kms-frw}.

\paragraph{Step 5: \(\varepsilon(a)\) law.}
Assuming locality of the kernel after averaging and the constitutive mapping \(\delta\ln M^2=\beta\,\delta\varepsilon\) (Sec.~\ref{sec:def-vs-map}), we obtain
\(
\frac{d\varepsilon}{d\ln a}=\sigma(a)\,\mathcal I(a)
\)
with \(\sigma(a)\ge 0\) and \(\mathcal I(a)\ge 0\) as in Sec.~\ref{sec:epsilon}.

\paragraph{Caveat.}
This derivation is preliminary: it assumes the conjectured analyticity of the averaged kernel and stability of Assumption C; a full microlocal/spectral proof is left to future work.
...
\item \texttt{referee\_pipeline.py} (FRW averaging module; \(\OmL=\beta f\cgeo\) cross-check; computes toy \(a_0=(5/12)\OmL^2 c H_0\)).
...
\item \texttt{s8\_hysteresis\_run.py} (BAO toy \(\chi_g\) sweeps).
\end{itemize}
Typical outputs include \texttt{epsilon\_evolution.png} and \texttt{bao\_growth.png} for the illustrative runs described in Secs.~\ref{sec:epsilon} and \ref{sec:env}. All Part II numerics are labeled \emph{toy/illustrative} and propagate the \(\pm 5\%\) \(\beta\) uncertainty into reported bands.
