...
\node[stage, below=of P1, draw=flowPurple!80, fill=flowPurple!8] (P3) {$\Omega_\Lambda \approx \beta f \, \cgeo$ with $\bar\varepsilon$ (Sec.~\ref{sec:sm-link})};
...
\node[stageH, below=8mm of G1, draw=flowGreen!80, fill=flowGreen!20, label={[flowOrange]left:CONDITIONAL}] (E0) {Entropy-driven $\bar\varepsilon(a)$ (Part II)};
\node[stage, below=of E0, draw=flowGreen!80, fill=flowGreen!8] (E1) {KMS/FDT positivity $\Rightarrow d\bar\varepsilon/d\ln a \ge 0$ (App.~\ref{app:entropic-proof})};
\node[stage, below=of E1, draw=flowGreen!80, fill=flowGreen!8] (E2) {Budget $\int \bar\varepsilon\, d\ln a = \Omega_\Lambda$ (Sec.~\ref{sec:epsilon})};
...
\end{tikzpicture}
\end{adjustbox}
\caption{Pipeline with PROVEN (blue/first green), CONDITIONAL (purple/second green/orange), and EXPLORATORY (red) elements. The theoremic core fixes \(\beta\) and the universal \(5/12\). The FRW mapping uses the \(f_V\)-weighted \(\bar\varepsilon\) (Sec.~\ref{sec:sm-link}). Part~III provides an \emph{exploratory} cluster-optics hook with an SK/BRSSS derivation path and a thermodynamic interpretation.}
\label{fig:pipeline}
\end{figure*}
...
% ===============================
% Cover letter (optional; not compiled)
\iffalse
