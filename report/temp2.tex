...
\textbf{Part II (Secs.~\ref{sec:def-vs-map}–\ref{sec:data}, Apps.~\ref{app:fv}–\ref{app:microlocal}):} conditional extensions, completed Lemmas C \& D, safe-window fraction, KMS\(\to\)FRW link, symmetry envelope, and toy/illustrative numerics with propagated uncertainties.
...
% ===============================
\section{Completed Proofs for Interacting Extensions at Working Order}
\label{sec:proofs}

\subsection{Lemma C (completed): Relative entropy \texorpdfstring{$\leftrightarrow$}{<->} canonical energy in the projected diamond}
\label{sec:lemmaC}
...
\ee
with \(\chi_{QK}^{\rm proj}\) positive-definite. The equality holds for interacting QFTs at working order \(\mathcal O(\ell^4)\).
\end{lemma}

\noindent\emph{Proof.}
(i) \textbf{Second variation as BKM metric.} For type III\(_1\) local algebras, Araki relative entropy is well-defined; near \(\sigma\), the second variation is the Bogoliubov--Kubo--Mori (quantum Fisher) metric evaluated on the tangent vector \(\dot\rho\) generated by the integrated perturbation \(\delta K_\sigma\) (Tomita--Takesaki theory). Thus
\(\ddot S|_0=\langle\!\langle \delta K_\sigma,\delta K_\sigma\rangle\!\rangle_{\rm BKM}\ge 0\).

(ii) \textbf{Projector and counterterm cancellation.} The MI/moment-kill projector eliminates contact terms and the \(r^0\), \(r^2\) moments. In Hadamard interacting QFTs, the state-dependent two-point Hadamard coefficients \(v,w\) enter only in terms canceled by the projector through \(\mathcal O(\ell^4)\) (App.~\ref{app:microlocal}, Prop.~\ref{prop:MIcancels}), leaving the flat-space coefficient.

(iii) \textbf{Ward identity and canonical energy.} The generator that implements the CHM conformal isometry is \(\xi_{\rm CHM}\). The quadratic form of the BKM metric for deformations generated by the stress tensor integrates to the canonical energy associated with \(\xi_{\rm CHM}\), \(\mathcal E_{\rm can}[\delta T;\xi_{\rm CHM}]\), after using the diffeomorphism Ward identity and the time-slice axiom. Projecting both sources by the MI/moment-kill channel yields \(\mathcal E_{\rm can}^{\rm proj}\).

(iv) \textbf{Positivity and working order.} Positivity follows from the KMS/BKM positivity and holds for the projected kernel \(\chi_{QK}^{\rm proj}\). All curvature/contact remainders are \(\mathcal O(\ell^6)\) by (ii). \qed

\subsection{Lemma D (completed): Uniqueness of the \texorpdfstring{$M^2$}{M^2} coupling at working order}
\label{sec:lemmaD}
...
\noindent\emph{Proof.}
Consider the most general local covariant functional at the required engineering dimension:
\be
\delta\mathcal L=\sqrt{-g}\left[a\,R+b\,R_{ab}R^{ab}+c\,\nabla^2 R+d\,\delta\ln M^2\,R
+ e\,\delta g^{00}+ f\,K\,\delta g^{00}+ \cdots\right],
\ee
where \(K\) is extrinsic curvature and ``\(\cdots\)" denote higher-derivative or parity-odd terms. Imposing \(c_T=1\) eliminates operators renormalizing the spin-2 kinetic term beyond \(M^2 R\); \(\alpha_B=0\) removes braiding operators proportional to \(\delta g^{00}K\). Isotropy and time-reversal exclude vector/tensor backgrounds. The MI/moment-kill projector cancels the \(r^0\), \(r^2\) moments and eliminates total derivatives such as \(\nabla^2R\); \(R\) and \(R_{ab}R^{ab}\) contributions are suppressed by \((\ell/L_{\rm curv})^2\) and higher, see App.~\ref{app:MI}. Thus the only unsuppressed scalar that couples linearly to the projected stress and deforms the growth sector without altering null distances is \(\delta\ln M^2\). Therefore the constitutive identification \(\delta\ln M^2=\beta\,\delta\eps\) is unique at working order within the stated symmetry/EFT class. \qed

\medskip
\noindent\emph{Remark.} Combining Lemmas C and D with the Part I theorem yields the conditional mapping \eqref{eq:mapping} with completed proofs at working order in the interacting Hadamard class under MI/moment-kill projection.
...
% ===============================
\section{Observational Illustrations (Toy; Uncertainty Propagated)}
\label{sec:obs}

\paragraph{Hubble ladder bounds (toy).}
We propagate the \(\pm 5\%\) uncertainty in \(\beta\) into \(\OmL\) and then into the toy \(H_0\) bounds. With \(\OmL=\beta f\cgeo=0.685\pm 0.034\), the previously quoted illustrative shifts \(H_0: 73.0\to 71.18\) (uncapped SN) and \(\to 70.89\) (capped SN+Cepheid) acquire \(\pm 0.17\)~km/s/Mpc systematic envelopes from \(\beta\), reported as
\be
...
\end{tikzpicture}
\caption{Semi-analytic \(f_V(\ell_{\min})\) at \(z\!\sim\!0\) for two excision parameters \(\xi\). Scripts in Sec.~\ref{sec:data}.}
\label{fig:fV}
\end{figure}
...
\(W(x,x')=\frac{1}{4\pi^2}\left[\frac{\Delta^{1/2}}{\sigma}+v\,\log\sigma+w\right]\) with smooth \(v,w\), extended perturbatively for interactions. The projector removes the \(F_0,F_2\) moments built from local counterterms, ensuring stability of the \(\ell^4\) coefficient (Lemma C).

\paragraph{OPE gap and log-falsifier.}
If an operator with protected dimension produces \(\ell^4\log\ell\) in the channel, the framework is falsified (explicit criterion in Sec.~\ref{sec:program}).
...
\end{itemize}
All Part II numerics are labeled \emph{toy/illustrative} and propagate the \(\pm 5\%\) \(\beta\) uncertainty into reported bands.
