...
The choice $s=[1+(\chi_g/\chi_\star)^q]^{-1}$ with \(q=2\) is a simple, Solar–System–compliant solution. Alternative monotone targets (e.g., \(q=1\), or using other curvature scalars) correspond to different \(\mathcal V(s;\chi_g)\) choices and will be constrained by data; our scripts allow these toggles. It should be regarded as a representative compliance function.
