...
\textbf{Part III (Sec.~\ref{sec:thermo}):} exploratory thermodynamic interpretation (Clausius form in the projected channel; conditional FRW budget) and relation to the Casini--Galante--Myers critique of Jacobson.
...
\paragraph{Thermodynamic analogy (working-order; projected channel).}
In the MI/moment-kill projected first-law channel, the entanglement first law 
$\delta S_{\rm sub}=\delta\!\langle K_{\rm sub}\rangle$ together with the BW KMS calibration 
$K=H_{\rm boost}/T_{\rm KMS}$ and $T_{\rm KMS}=\kappa/(2\pi)$ implies a Clausius-like identity
$\delta S_{\rm sub}=\delta Q_{\rm boost,sub}/T_{\rm KMS}$ at working order. Using 
$\delta\!\langle K_{\rm sub}\rangle=\beta\,\ell^4\,\delta\varepsilon+\mathcal O(\ell^6)$, we obtain
$\delta S_{\rm sub}=\beta\,\ell^4\,\delta\varepsilon$, i.e.\ a thermodynamic interpretation without 
macroscopic heat. Under the hypotheses stated above, the FRW zero mode then yields the 
\emph{conditional} budget $\int\varepsilon\,d\ln a=\Omega_\Lambda=\beta f c_{\rm geo}$. 
This differs from Jacobson’s entanglement-equilibrium program \cite{Jacobson2016}: the 
Casini--Galante--Myers critique \cite{CasiniGalanteMyers2016} identifies logarithmic issues for 
marginal $\Delta=d/2$ operators, whereas our MI/moment-kill channel targets the $\ell^4$ response 
(with no $\ell^4\log\ell$ at working order). Any observed $\ell^4\log\ell$ residue in this channel 
would falsify our working-order assumptions (Sec.~\ref{sec:program}).
...
In the MI/moment-kill projected first-law channel we have $\delta S_{\rm sub}=\delta\!\langle K_{\rm sub}\rangle$.
With the BW KMS normalization $K=H_{\rm boost}/T_{\rm KMS}$ and $T_{\rm KMS}=\kappa/(2\pi)$, 
\be
\delta S_{\rm sub}=\frac{\delta Q_{\rm boost,sub}}{T_{\rm KMS}} \,, 
\qquad
\delta Q_{\rm boost,sub}\equiv\delta\!\langle H_{\rm boost,sub}\rangle .
\ee
Since $\delta\!\langle K_{\rm sub}\rangle=\beta\,\ell^4\,\delta\varepsilon+\mathcal O(\ell^6)$ (Theorem~\ref{thm:proj-modresp}), 
it follows that $\delta S_{\rm sub}=\beta\,\ell^4\,\delta\varepsilon+\mathcal O(\ell^6)$. 
This is a Clausius-type relation derived from KMS/FDT and the entanglement first law, with “heat” identified 
as boost energy in the projected channel.

\subsection{FRW Clausius extension (conditional proposition)}
Under the KMS$\to$FRW hypotheses of Sec.~\ref{sec:kms-frw} (locality/decay, adiabaticity, analyticity under 
$s\to\ln a$, diamond--half-space control), the FRW-averaged susceptibility reduces to a contact term in $\ln a$,
leading to the \emph{conditional} normalization
\be
\int_{a_i}^{1}\varepsilon(a)\,d\ln a=\Omega_\Lambda=\beta f\,c_{\rm geo}\,.
\ee
This can be viewed as a coarse-grained Clausius budget. Violations (e.g.\ non-local kernels, failure of analyticity) 
would appear as residual non-contact terms and are covered by the falsifiers in Sec.~\ref{sec:program}.

\subsection{Relation to Jacobson (2016) and the CGM critique}
Jacobson’s “entanglement equilibrium” proposal ties a local Clausius statement to the Einstein equation 
\cite{Jacobson2016}. Casini--Galante--Myers (CGM) showed that for relevant deformations with low scaling dimensions, 
and in particular for \emph{marginal} $\Delta=d/2$ cases, logarithmic contributions obstruct a straightforward 
universal inference \cite{CasiniGalanteMyers2016}. Our framework differs in two respects: (i) we do not attempt a 
universal derivation of GR; (ii) the MI/moment-kill projector cancels $r^0$ and $r^2$ moments, and our working-order 
channel targets the $\ell^4$ response \emph{without} an $\ell^4\log\ell$ term. Accordingly, we \emph{sidestep} (rather 
than resolve) the marginality issue: the presence of an $\ell^4\log\ell$ contribution in the projected channel would 
falsify our assumptions (Sec.~\ref{sec:program}), while its absence keeps the Clausius identity and the conditional FRW 
extension intact. (CHM’s hyperbolic map \cite{Casini2011} underlies the KMS calibration we employ.)

\paragraph{Marginal ($\Delta=d/2$) running (exploratory).}
In interacting QFTs a protected marginal operator ($\Delta=d/2=2$ in $d=4$) can induce
$\ell^4\!\ln(\ell/\ell_0)$ corrections in the MI/moment–kill channel. This is naturally
interpreted as a mild running of the flat–space coefficient,
\[
\beta(\ell)=\beta_0\!\left[1+\alpha_{\log}\,\ln(\ell/\ell_0)+\mathcal O(\alpha_{\log}^2)\right],
\qquad |\alpha_{\log}|\ll 1.
\]
Part~I (Gaussian/Hadamard) yields $\alpha_{\log}=0$ at working order (consistent with the
MI–subtracted plateau), while in Part~II we allow bounded running as a conditional extension.
Equations~\eqref{eq:eps-def}, \eqref{eq:eft-stub}, and \eqref{eq:budget} then hold with
$\beta\to\beta(\ell_\ast)$ for any $\ell_\ast$ in the safe window, e.g.
\[
\mu(\varepsilon)=\frac{1}{1+\tfrac{5}{12}\,[1+\alpha_{\log}\ln(\ell_\ast/\ell_0)]\,\varepsilon},
\qquad 
\int\varepsilon\,d\ln a=\Omega_\Lambda=\beta(\ell_\ast)\,f\,\cgeo.
\]
Detecting a residual $\ell^4\ln\ell$ trend in the projected channel would falsify the
working–order assumption (Assumption~C test), whereas $|\alpha_{\log}|\!\ll\!1$ defines the
``$\Delta=d/2$ marginal thermodynamics’’ program to be constrained by data.
...
\(\alpha_{\log}\) & (Optional) marginal running: \(\beta(\ell)=\beta_0[1+\alpha_{\log}\ln(\ell/\ell_0)+\cdots]\) \\
