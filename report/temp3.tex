...
Matter obeys the standard continuity and Euler equations. This closure preserves the Bianchi identity at working order because \(s(x)\) is a scalar; an action-level realization and frame‑independence are given below (Remark~\ref{sec:throttle-remark}).
...
\noindent\emph{Lensing departure bound.}
Gradient corrections induce $|\Sigma-1|\sim \mathcal O\!\big((\ell/L_{\rm curv})^2\big)$; with $\ell\sim 10$–$100\,\mathrm{pc}$ and $L_{\rm curv}\sim 0.3$–$1\,\mathrm{Mpc}$ in clusters, this yields $|\Sigma-1|\lesssim 10^{-10}$–$10^{-7}$, negligible for merger-lensing morphologies.
...
\ee
Using the baseline \(\OmL=0.685\) and \(H_0=70.9~\mathrm{km\,s^{-1}\,Mpc^{-1}}\), this gives \(a_0^{\rm eff}\approx 1.2\times 10^{-10}\,\mathrm{m\,s^{-2}}\) in the weak-field limit (\(s\simeq 1\)), and the effective $a_0^{\rm eff}$ is \emph{enhanced} in weak-field regimes by the \emph{derived} $s\!\to\!1$ (not imposed), while Solar–System compliance follows from $s(\chi_\odot)\ll 1$ (Sec.~\ref{sec:env}); pipeline values propagate the \(\pm 5\%\) uncertainty in \(\beta\).
...
\paragraph{Operational tests (pass/fail).}
\begin{itemize}[leftmargin=*,noitemsep,topsep=0pt]
\item \textbf{GR-like distances:} EM/GW luminosity distances agree at working order, $|d_L^{\rm GW}/d_L^{\rm EM}-1|\lesssim5\times 10^{-3}$. \emph{Fail} if a lensing coupling $\Sigma\neq1$ is required.
...
\subsection{Merging clusters and void-wall lensing: diagnostics and nulls (working order)}
\label{sec:clusters-voids}

\paragraph{Bullet-type mergers (prediction).}
At working order we have $\Sigma\simeq 1$, hence the lensing potential $(\Phi+\Psi)/2$ obeys the GR Poisson law and $\kappa$ traces \emph{true} projected mass. In shock-heated merger geometries the X-ray gas dominates the projected baryon mass near the shock; throttling alters $\Phi$ (dynamics) through $\mu(\varepsilon,s)$ but \emph{does not} move lensing peaks. Therefore a Bullet-like $\kappa$–X-ray offset requires collisionless mass.

\paragraph{Neutrino upper bound (first-principles).}
With $\sum m_\nu\lesssim 0.12\,\mathrm{eV}$ and $h\simeq 0.7$ one has
\[
\Omega_\nu=\frac{\sum m_\nu}{93.14\,h^2\,\mathrm{eV}}\simeq 2.6\times 10^{-3},\qquad
\frac{\Omega_\nu}{\Omega_m}\simeq 0.9\% .
\]
In a $10^{15}M_\odot$ system this is $\sim 9\times 10^{12}M_\odot$, at least an order of magnitude below the $\sim 10^{14}M_\odot$ per subcluster required to shift $\kappa$ peaks from the shocked gas to the galaxy subclusters in simple projection models. Standard-Model neutrinos cannot reproduce the offset under $\Sigma\simeq 1$.

\paragraph{Void-wall lensing (target signal).}
In low-curvature regions ($s\to 1$) throttling reduces growth while keeping distances GR-like; stacked void-wall profiles are therefore the natural arena for percent-level lensing modulations. Our toy runs predict $\Delta\kappa/\kappa\sim 1$–$2\%$ near BAO/void shells, compensated by off-peak suppression (Sec.~\ref{sec:env}).
...
\textbf{Lemma G (Nonlinear validation)}: Initial Gadget-4 runs are complete (baseline resolution; \texttt{gadget4\_mu\_eps\_toy.py}); post-processing and archiving (Zenodo DOI) are pending. These test \(\mu(\varepsilon,s)\) and \(s(\chi_g)\) effects on structure formation and lensing, with BAO features and lensing shear targeted.\\
\textbf{Falsifiers:} (i) persistent \(\ell^4\log\ell\) residuals in the projector channel; (ii) GW/EM distance ratio beyond \(5\times 10^{-3}\); (iii) \(|\dot G/G|\gtrsim 10^{-12}\,\mathrm{yr}^{-1}\); (iv) \(\OmL\) inconsistent with \(\beta f\cgeo\); (v) \(S_8\) outside the extremal band for all admissible monotone \(\eps(a)\) satisfying the budget; (vi) positivity failure in Assumption C tests; (vii) detection of $|\Sigma-1|\gtrsim 10^{-3}$ in low-curvature (void-wall) environments; (viii) a Bullet-type $\kappa$–X-ray offset reproducible with purely baryonic mass under $\Sigma\simeq 1$.
...
\item \textbf{Nonlinear validation:} full N-body and ray-tracing tests for \(\mu(\varepsilon,s)\) and \(s(\chi_g)\), including BAO-scale modulation and lensing systematics.
...
\end{tikzpicture}
\end{adjustbox}
\caption{Pipeline with PROVEN (blue/first green), CONDITIONAL (purple/second green/orange), and EXPLORATORY (red) elements. The theoremic core fixes \(\beta\) and the universal \(5/12\). The FRW mapping and budget are \emph{conditional} (Sec.~\ref{sec:kms-frw}). Part~III provides an \emph{exploratory} thermodynamic interpretation and clarifies the relation to the CGM critique.}
\label{fig:pipeline}
\end{figure*}
...
\item \texttt{bullet\_void\_diagnostics.py} (2D toy $\kappa$ maps for Bullet-like geometries; neutrino surface-density upper bound; void-wall stacking forecast). Produces \texttt{kappa\_toy\_grid.png} and summary tables used in Sec.~\ref{sec:clusters-voids}.
\end{itemize}
...
We submit the revised manuscript “Modular Response in Free Quantum Fields: A KMS/FDT Theorem and Conditional Extensions” (dated August 30, 2025). This revision implements the minor updates recommended by the referees and aligns with Referee 7’s request for stronger physical foundations and explicit falsifiers:

\begin{itemize}
  \item We promote the \emph{Frame-independence of throttling} to a subsection (Sec.~\ref{sec:throttle-remark}) with a covariant, action-level realization of the environment modulation $begin:math:text$s(x)$end:math:text$, an explicit Jordan$begin:math:text$\\leftrightarrow$end:math:text$Einstein map, and an explicit \emph{scale-separation} note clarifying how the local modular response propagates to cosmology.
  \item We remove the ad-hoc gate in favor of the \emph{action-derived} envelope $begin:math:text$s(x)$end:math:text$, and we calibrate Solar–System compliance (Table~\ref{tab:compliance}).
  \item We clarify the weak-field enhancement of the MOND-like scale by quoting a numerical baseline $begin:math:text$a_0^{\\rm eff}\\approx 1.2\\times 10^{-10}\\,\\mathrm{m\\,s^{-2}}$end:math:text$ (Sec.~\ref{sec:throttle-remark}).
  \item We add a robustness note on \textbf{alternative envelopes} for $begin:math:text$s(x)$end:math:text$ (including an exponential option) and expose toggles in \texttt{cosmology\_runner.py}.
  \item We polish editorial items: capitalized “Boost-Energy Variation” in the Symbol Index and added a vertical-axis label (“Workflow Progression”) to Fig.~\ref{fig:pipeline}.
\end{itemize}

\noindent \textbf{Falsifiability:} We emphasize explicit failure modes (Sec.~\ref{sec:program}): GW/EM distance splits, bounds on $begin:math:text$|\\dot G/G|$end:math:text$, an $begin:math:text$\\ell^4\\log\\ell$end:math:text$ signal in the MI/moment-kill channel, and violations of the FRW budget normalization.

\noindent \textbf{Data \& Code:} All analysis scripts and single-file runners are archived on Zenodo (Zenodo DOI: \texttt{\zenododoi}). The archive includes \texttt{beta\_methods\_v2.py}, \texttt{cosmology\_runner.py}, \texttt{referee\_pipeline.py}, \texttt{fv\_semi\_analytic.py}, \texttt{gadget4\_mu\_eps\_toy.py}, and \texttt{s8\_hysteresis\_run.py}, each with usage notes to reproduce the figures and tables.
